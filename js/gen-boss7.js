...
(function () {
	...
	boss('boss7-cosine-sine-chain', 'Cosine rule and sine rule chain', function (r) {
		const a = r.int(6, 15), b = r.int(6, 15), A = r.pick([40, 50, 60, 70, 80]); const c = Math.sqrt(a * a + b * b - 2 * a * b * Math.cos(rad(A))), B = Math.acos((a * a + c * c - b * b) / (2 * a * c)) * 180 / Math.PI;
		return { marks: 7, text: `In triangle ABC, AB = ${a} cm, AC = ${b} cm and the included angle A = ${A}&deg;; let BC = c. Give answers correct to 2 decimal places.${br('i', 'Find c by the cosine rule.')}${br('ii', 'Find the area.')}${br('iii', 'Use the sine rule to find angle B.')}`, diagram: { type: 'triangle', a: c, b, c: a, labels: ['A', 'B', 'C'], sideLabels: ['c', `${b} cm`, `${a} cm`], angleLabels: [`${A}&deg;`, '', ''] }, answer: multi([dp(c, 2), dp(0.5 * a * b * Math.sin(rad(A)), 2), dp(B, 2)], ['c (2 d.p.)', 'area (2 d.p.)', 'angle B (2 d.p.)'], 0.011), solution: `<p>Cosine rule: c&sup2; = ${a}&sup2; + ${b}&sup2; &minus; 2(${a})(${b})cos ${A}&deg;.</p><p>(i) c = <strong>${dp(c, 2)}</strong>.</p><p>(ii) Area = &frac12;(${a})(${b})sin ${A}&deg; = <strong>${dp(0.5 * a * b * Math.sin(rad(A)), 2)}</strong>.</p><p>(iii) sin B/${b} = sin ${A}&deg;/${dp(c, 4)}. Choose the angle consistent with the side lengths: <strong>B = ${dp(B, 2)}&deg;</strong>.</p>` };
	});
