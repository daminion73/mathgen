...
(function () {
	...
	const sk = (fn, w, xIntercepts = [], breaks = []) => ({ type: 'sketch', fn, ...w, xIntercepts: [...new Set(xIntercepts)], yIntercept: Number.isFinite(fn(0)) ? fn(0) : null, breaks });
	...
	add('sketch-triple-branch', 'Rational curves', 2, (r) => {
		...
		return { marks: 5, text: `Sketch y=${F.frac(k, `${fac(a)}${fac(b)}`)}. Draw all three branches, using the vertical and horizontal asymptotes to determine their behaviour.`, answer: sk(fn, w, [], [a, b]), solution: `<p>The denominator gives vertical asymptotes x=${a}, ${b}; y=0 is horizontal. Use a sign table in the three intervals.</p>${graphSol(fn, w, { vlines: [{ x: a }, { x: b }], hlines: [{ y: 0 }] })}` };
	});
	...
	add('sketch-linear-over-quadratic', 'Rational curves', 3, (r) => {
		...
		return { marks: 7, text: `Sketch y=${s < 0 ? '&minus;' : ''}${F.frac(fac(z), `${fac(a)}${fac(b)}`)}. Include its intercepts and every branch, working out the asymptotes yourself.`, answer: sk(fn, w, [z], [a, b]), solution: `<p>The intercept is x=${z}; poles are x=${a}, ${b}; y=0 is the end asymptote. A sign chart fixes each branch.</p>${graphSol(fn, w, { vlines: [{ x: a }, { x: b }], hlines: [{ y: 0 }] })}` };
	});
	...
	add('sketch-reciprocal-bell', 'Reciprocal quadratics', 2, (r) => {
		...
		return { marks: 4, text: `Sketch y=${F.frac(A, `${fac(h)}${F.sup(2)}+${c}`)}, including the maximum and correct behaviour towards the horizontal asymptote.`, answer: sk(fn, w), solution: `<p>The curve is positive and symmetric about x=${h}. Its maximum is ${F.pt(h, A / c)} and it tapers to y=0.</p>${graphSol(fn, w, { hlines: [{ y: 0 }] })}` };
	});

	add('sketch-reciprocal-parabola', 'Reciprocal reasoning', 3, (r) => {
		const [a, b] = pair(r), s = r.sign(), w = win(-5, 5, -6, 6), f = (x) => s * (x - a) * (x - b), fn = (x) => 1 / f(x);
		return { marks: 7, text: `The displayed curve is y=f(x). On the answer grid sketch y=${F.frac(1, 'f(x)')}, using the asymptotes to position every branch with its correct sign.`, diagram: { type: 'graph', ...w, grid: true, fns: [{ fn: f, color: '#444' }], points: [[a, 0, 'A'], [b, 0, 'B']] }, answer: sk(fn, w, [], [a, b]), solution: `<p>The zeros ${a}, ${b} become vertical asymptotes. Reciprocal values retain the sign of f and tend to zero away from the roots.</p>${graphSol(fn, w, { vlines: [{ x: a }, { x: b }], hlines: [{ y: 0 }] })}` };
	});
	...
	add('sketch-absolute-cubic', 'Modulus transformations', 3, (r) => {
		...
		return { marks: 6, text: `The diagram shows f(x)=${fac(a)}${fac(b)}${fac(c)}. Sketch y=|f(x)| on the answer grid, retaining all intercepts.`, diagram: { type: 'graph', ...w, grid: true, fns: [{ fn: base, color: '#777' }] }, answer: sk(fn, w, [a, b, c]), solution: `<p>Keep the parts above the x-axis and reflect every part below it upwards.</p>${graphSol(fn, w)}` };
	});
	...
	add('sketch-shifted-semicircle', 'Restricted curves', 2, (r) => {
		...
		return { marks: 4, text: `Sketch the ${s > 0 ? 'upper' : 'lower'} semicircle y=${k}${s > 0 ? '+' : '&minus;'}${F.sqrt(`${rad * rad}&minus;${fac(h)}${F.sup(2)}`)}, marking both endpoints and the extreme point.`, answer: sk(fn, w, zeros), solution: `<p>It belongs to the circle centred at ${F.pt(h, k)} with radius ${rad}; retain only the stated half.</p>${graphSol(fn, w)}` };
	});
	...
	add('sketch-shifted-exponential', 'Exponential curves', 2, (r) => {
		...
		return { marks: 4, text: `Sketch y=${base}${F.sup(`${s < 0 ? '&minus;' : ''}${fac(h)}`)}${F.st(k, '')}, including the point where x=${h} and correct behaviour towards its horizontal asymptote.`, answer: sk(fn, w, k < 0 ? [h + Math.log(-k) / (s * Math.log(base))] : []), solution: `<p>The horizontal asymptote is y=${k}; the translated key point is ${F.pt(h, k + 1)}. The sign in the exponent determines growth direction.</p>${graphSol(fn, w, { hlines: [{ y: k }] })}` };
	});
	...
	add('sketch-shifted-log', 'Logarithmic curves', 3, (r) => {
		...
		return { marks: 5, text: `Sketch y=log<sub>${base}</sub>${fac(h)}${F.st(k, '')}, including a translated key point and correct behaviour near the vertical asymptote.`, answer: sk(fn, w, [h + base ** -k], [h]), solution: `<p>The asymptote is x=${h}; translating (1,0) gives ${F.pt(h + 1, k)}.</p>${graphSol(fn, w, { vlines: [{ x: h }] })}` };
	});
	...
	add('sketch-oblique-reciprocal', 'Addition of ordinates', 3, (r) => {
		...
		return { marks: 7, text: `Sketch y=${F.lt(m, 'x')}${F.st(c, '')}${F.st(q, '')}/${fac(a)}. Work out the vertical and oblique asymptotes to position both branches.`, answer: sk(fn, w, roots([m, c - m * a, q - c * a]), [a]), solution: `<p>The reciprocal term gives x=${a}; as it tends to zero the curve approaches y=${F.lt(m, 'x')}${F.st(c, '')}.</p>${graphSol(fn, w, { vlines: [{ x: a }], fns: [{ fn }, { fn: (x) => m * x + c, color: 'var(--dg-faint, #8c8c8c)' }] })}` };
	});
	...
	add('sketch-quadratic-plus-reciprocal', 'Addition of ordinates', 3, (r) => {
		...
		return { marks: 8, text: `Sketch y=${fac(h)}${F.sup(2)}${F.st(q, '')}/${fac(a)}. Determine the vertical asymptote and use the parabolic end behaviour to shape the branches.`, answer: sk(fn, w, roots([1, -a - 2 * h, h * h + 2 * a * h, q - a * h * h]), [a]), solution: `<p>The pole is x=${a}. For large |x| the reciprocal term disappears, so the curve approaches y=${fac(h)}${F.sup(2)}.</p>${graphSol(fn, w, { vlines: [{ x: a }] })}` };
	});
})();
